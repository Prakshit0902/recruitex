...
const JobSeekerLandingPage = () => {
    ...
    const fetchFeaturedJobs = useCallback(async () => {
        try {
            const res: any = await axios.get(`${job_service}/api/job/jobs`);
            const jobsList = res.data.jobs ?? res.data;
            setJobs(jobsList.slice(0, 6));
        } catch (error) {
                ...
                <div className="jl-container">
                    ...
                    <div className="jl-grid-3">
                        {jobsLoading ? (
                            ...
                            jobs.map((job) => (
                                <div className="jl-card" key={job.jobId} style={{ padding: 22, display: "flex", flexDirection: "column" }}>
                                    {/* Company logo + name */}
