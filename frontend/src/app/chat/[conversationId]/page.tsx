...
const ChatConversationPage = () => {
        ...
        const handleNewMessage = (message: Message) => {
            if (message.conversationId === conversationId) {
                setMessages((prev) => {
                    ...
                    if (prev.some((m) => !m._isOptimistic && m.messageId === message.messageId)) {
                        return prev;
                    }
                    // Replace the optimistic placeholder from the same sender with the same content
                    const optimisticIndex = prev.findIndex(
                        (m) => m._isOptimistic && m.senderId === message.senderId && m.content === message.content
                    );
    ...
    const handleSend = () => {
        ...
        const optimisticMsg: Message = {
            messageId: Date.now(), // temporary id; replaced when server echo arrives
