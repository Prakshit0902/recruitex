...
const CompanyPage = () => {
  ...
  async function fetchCompany() {
    try {
      ...
      // Backend returns { company: {..., jobs: [...] } }
      setCompany((data.company ?? data) as any);
    } catch (error) {
      console.log(error);
    } finally {
      setLoading(false);
  ...
  const isRecruiterOwner =
    user && company && user.userId === company.recruiterId;
  ...
  const addJobHandler = async () => {
    ...
    try {
      ...
      await axios.post(`${job_service}/api/job/job/new`, jobData, {
        headers: {
  ...
  const deleteHandler = async (jobId: number) => {
    if (confirm("Are you sure you want to delete this job?")) {
      setBtnLoading(true);
      try {
        await axios.delete(`${job_service}/api/job/jobs/${jobId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleOpenUpdateModal = (job: Job) => {
    ...
    setjob_type(job.jobType);
    setwork_location(job.workLocation);
    setis_active(job.isActive);
  ...
  const updateJobHandler = async () => {
    ...
    try {
      ...
      await axios.put(
        `${job_service}/api/job/jobs/${selectedJob.jobId}`,
        updateData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
                        ...
                        <div className="flex items-start justify-between gap-4 flex-wrap">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3 mb-3 flex-wrap">
                              ...
                              <span
                                className={`text-xs px-3 py-1 rounded-full flex items-center gap-1 ${j.isActive
